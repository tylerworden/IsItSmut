# Seed Batch 3 (300 Titles) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Triple the rated catalog (~150 → ~450 pages) with a broad-mix 300-title seed batch per `docs/superpowers/specs/2026-08-02-isitsmut-seed-batch-3-design.md`.

**Architecture:** One new operator script, `scripts/seed-popular-3.ts`, identical in shape to `scripts/seed-popular-2.ts` (hardcoded `SEED` list run sequentially through the real `runDisambiguate` → `runRate` flow) plus a `[n/300]` progress counter. No app-code, test, or schema changes.

**Tech Stack:** TypeScript, tsx runner, Supabase (prod), Anthropic Claude via existing `src/lib` flow.

## Global Constraints

- Package manager: `pnpm` (`pnpm typecheck`), run from `C:\Users\tword\desktop\isitsmut`.
- TLS-intercepting machine: `pnpm dlx` is BLOCKED. Operator scripts run via `NODE_TLS_REJECT_UNAUTHORIZED=0 npm_config_strict_ssl=false npx --yes tsx@latest --env-file=.env.local scripts/<file>.ts`.
- Operator scripts have NO unit tests by design (they execute on import; `--scan`-style dry paths and `pnpm typecheck` are the gates — same convention as `seed-popular-2.ts` and `merge-duplicates.ts`).
- Script-only changes commit directly to `main` and push (established 2026-08-02 with `seed-popular-2.ts`, commit `7a54f54`). No PR needed; the app deploy is unaffected.
- Running the script spends real Anthropic API money (~$5–10) and writes to prod Supabase. Run it exactly once, in the background, after the commit lands.
- Commit messages end with `Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>`.
- **Freshness-pass deviation from spec (intentional, decided during planning):** the rating model's knowledge boundary is ~spring 2025 (proven by batch 2: its only unknowns were May/Sept 2025 releases), so "current titles" means trending 2024–early-2025 releases the model can recognize (30+ of them are in the list below), plus exactly 4 buzzy 2026 releases kept as future-proofing — expected to return `known:false` (noindex, harmless) until models update, then recoverable via `cleanup-rerate.ts`.

---

### Task 1: Create `scripts/seed-popular-3.ts`

**Files:**
- Create: `scripts/seed-popular-3.ts`

**Interfaces:**
- Consumes: `runDisambiguate(query: string)` from `../src/lib/disambiguate`, `runRate({ slug, candidate })` from `../src/lib/rate`, `Medium` type from `../src/lib/types` — all unchanged, same imports as `seed-popular-2.ts`.
- Produces: nothing importable (top-level operator script, executes on import). Task 2 runs it via the operator command.

- [ ] **Step 1: Write the file**

Create `scripts/seed-popular-3.ts` with exactly this content:
...
- [ ] **Step 2: Verify no duplicates within or across seed batches**

Run (Git Bash syntax):

```bash
grep -h "query: '" scripts/seed-popular.ts scripts/seed-popular-2.ts scripts/seed-popular-3.ts | sort | uniq -d
```

Expected: NO output (no query line appears twice anywhere). Also confirm the new file has exactly 300 entries:

```bash
grep -c "query: '" scripts/seed-popular-3.ts
```

Expected: `300`. If either check fails, fix the SEED list before proceeding.

- [ ] **Step 3: Typecheck**

Run: `pnpm typecheck`
Expected: exit 0. (Operator scripts are covered by tsc; there are no unit tests for them by design — see Global Constraints.)

- [ ] **Step 4: Commit and push**

```bash
git add scripts/seed-popular-3.ts
git commit -m "feat(scripts): third seed batch — 300 titles, broad mix (bestsellers/YA/romance/screen)"
git push
```

---

### Task 2: Run the batch against prod

**Files:**
- No file changes. Runs `scripts/seed-popular-3.ts` once.

**Interfaces:**
- Consumes: the committed script from Task 1; prod env in `.env.local`.
- Produces: ~300 rated rows in prod `works`/`ratings`; a run log whose tally Task 3 reads.

- [ ] **Step 1: Launch in the background**

Run (Git Bash syntax, as a background task — expect a few hours):

```bash
NODE_TLS_REJECT_UNAUTHORIZED=0 npm_config_strict_ssl=false npx --yes tsx@latest --env-file=.env.local scripts/seed-popular-3.ts
```

- [ ] **Step 2: On completion, extract the tally and every non-ok line**

From the run log, collect the final `=== Done — ok: …` line plus every `✗` and `⚠` line.
Expected: ok ≥ ~280; unknown ≈ 4–10 (the four 2026 future-proof titles, plus possibly a few mid-2025 releases like Problematic Summer Romance); failed = 0. The script is idempotent — if it crashes mid-run, re-run the same command; completed titles become free cache hits.
...
**Files:**
- Modify: `C:\Users\tword\.claude\projects\C--Users-tword-desktop-isitsmut\memory\PROJECT-HANDOFF.txt` (memory, outside the repo)

**Interfaces:**
- Consumes: Task 2's run log; `scripts/merge-duplicates.ts` (committed on main).

- [ ] **Step 1: Duplicate scan must stay clean**
...
Expected: `No duplicate groups found.` If groups appear, the disambiguate slug-reuse fix regressed or a title-variant slipped through — report to Tyler with the scan output before doing anything else; do NOT run `--merge` without review.

- [ ] **Step 2: Spot-check three newly seeded pages live**

Pick three fresh (non-`(cached)`) slugs from the run log — one book, one movie, one TV — and:

```bash
curl -s --ssl-no-revoke https://isitsmut.com/r/<slug> | grep -o '<title>[^<]*</title>'
```

Expected: each returns the tease-format title (`Is <Title> Smut? Spice Level & Scene Guide — IsItSmut`), confirming the pages render and are indexable.

- [ ] **Step 3: Update the handoff memory**

In `PROJECT-HANDOFF.txt`, update the seed-batch note: batch 3 (300 titles) run on 2026-08-02, final tally, catalog now ~N pages (from the tally), unknowns listed by name for future `cleanup-rerate.ts` recovery when models update.

- [ ] **Step 4: Report the tally to Tyler**

Final message: ok/unknown/failed counts, catalog size, any surprises (unexpected unknowns, medium mismatches worth eyeballing).
